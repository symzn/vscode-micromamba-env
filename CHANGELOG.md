...
-   **Python Detection on Linux**: Fixed a bug where the temporary watcher for new environments failed on Linux by monitoring an incorrect path for the Python executable. The watcher now correctly uses the relative path to ensure reliable detection.
